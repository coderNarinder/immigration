...
<div class="wrapper-shadow-inset">
  ...
  <form class="form-edit-category" name="addCategoryEdit" role="form" method="post" id="categoryForm" 
  data-action="{{ route('admin.users.edit',$user->id) }}"
  enctype="multipart/form-data">
   @csrf
    ...
    <div class="row">
          ...
          <div class="col-sm-6">{{textbox($errors,'Name*','name',$user->name)}}</div>
          <div class="col-sm-6">{{textbox($errors,'Email*','email',$user->email)}}</div>
          <div class="col-sm-6">{{textbox($errors,'Phone Number*','phone_number',$user->phone_number)}}</div>
        ...
        <label class="filelabel">
         ...
         @if(!empty($user->profile_picture))
           <img src="{{url($user->profile_picture)}}" class="img-thumbnail" style="width:120px;">
         @endif    
          </div>
          </div>
