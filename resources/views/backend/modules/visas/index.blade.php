...
@section('content')
<div class="row px-4 pt-3 profile dash mb-4"><h2 class=" mb-2">View VisaDetail</h2></div>
            ...
            <div class="row py-4 px-3">
                <div class="row pb-3 px-3">
                  ...
                  <div class="col-lg-3 col-2 pt-3">
                    <a href="{{route('admin.VisaDetail.create')}}" class="enq">
                ...
                <div class="table table-responsive px-3">
                  ...
                  <tr align="center" valign="middle" >
                   <td><img src="/{{$val->image}}" style="width:120px"></td>
                   <td>{{$val->type->name ?? '--'}}</td>
                   <td>{{$val->name}}</td>
                   <td>{{$val->country->name ?? '--'}}</td>
                   <td width="200px">
                      <select class="form-select changeStatus">
                        <option value="{{route('admin.VisaDetail.status',[$val->id,1])}}" <?= $val->status == 1 ? 'selected' : '' ?>>Active</option>
                        <option value="{{route('admin.VisaDetail.status',[$val->id,0])}}" <?= $val->status == 0 ? 'selected' : '' ?>>In-Active</option>
                    </select>
                  </td> 
                  <td align="center">
                    <div class="drop">
                      <a href="" class="d-flex align-items-center text-black text-decoration-none" id="dropdownUser1" data-bs-toggle="dropdown" aria-expanded="false"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-three-dots-vertical" viewBox="0 0 16 16">
                        ...
                        <div class="dropdown pull-right">
                          <ul class="dropdown-menu dropdown-menu-light text-small shadow" aria-labelledby="dropdownUser1">
                            <li><a class="dropdown-item" href="{{route('admin.VisaDetail.edit',$val->id)}}">Edit</a></li>
